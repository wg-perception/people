...
#places
echo "call 1"
${BIN}/buildTrainingImageDataset -set svlVision.svlImageLoader channels "INTENSITY .png DEPTH .dm" -set svlVision.svlImageLoader defaultExtension .png -set svlVision.svlTrainingDatasetBuilder objects cups -set svlVision.svlTrainingDatasetBuilder resizeWidth 32 -set svlVision.svlTrainingDatasetBuilder resizeHeight 64 -set svlVision.svlTrainingDatasetBuilder baseDir data/objects${NAME}-direct-test ${TEST_SEQ} ${TEST_LAB} 
echo "/call 1"
...
#export line=-1.0
    ...
    echo "Building patch dictionary..."
     ${BIN}/buildPatchDictionary -set svlVision.svlImageLoader channels "INTENSITY .png EDGE .png DEPTH .dm" -n 10 -o models/cups${NAME}${line}.dictionary.xml data/objects${NAME}${line}/cups/ 32 64
...
echo "First call to buildPatchResponseCache"
    ${BIN}/buildPatchResponseCache -set svlVision.svlImageLoader channels "INTENSITY .png EDGE .png DEPTH .dm" -maxImages 60000 data/objects${NAME}${line}/cups/ /tmp/cache-cups-pos${NAME}${line} models/cups${NAME}${line}.dictionary.xml
echo "Second call to buildPatchResponseCache"
    ${BIN}/buildPatchResponseCache -v -set svlVision.svlImageLoader channels "INTENSITY .png EDGE .png DEPTH .dm" -maxImages 60000 data/objects${NAME}${line}/negative/ /tmp/cache-cups-neg${NAME}${line} models/cups${NAME}${line}.dictionary.xml
...
echo "Evaluating performance on the test set"
    ${BIN}/classifyImages -n cups -o /tmp/detections${NAME}${line}.xml -t 0.5 -v -set svlVision.svlImageLoader channels "INTENSITY .png EDGE .png DEPTH .dm" models/cups${NAME}${line}.dictionary-trimmed.xml models/cups${NAME}${line}-trimmed.model ${TEST_SEQ}
...
echo "Evaluating performance on the train set"
    ${BIN}/classifyImages -n cups -o /tmp/detections_train${NAME}${line}.xml -t 0.05 -v -baseExt .png -set svlVision.svlImageLoader channes "INTENSITY .png -ch EDGE .png DEPTH .png" models/cups${NAME}${line}.dictionary-trimmed.xml models/cups${NAME}${line}.model ${SEQ}
...
${BIN}/buildPatchResponseCache -ch INTENSITY .png -ch EDGE .png -maxImages 6000000 data/objects${NAME}-direct-test/cups /tmp/cache-cups-pos${NAME}${line}-direct-test models/cups${NAME}${line}.dictionary-trimmed.xml
    ${BIN}/buildPatchResponseCache -ch INTENSITY .png -ch EDGE .png -maxImages 6000000 data/objects${NAME}-direct-test/negative /tmp/cache-cups-neg${NAME}${line}-direct-test models/cups${NAME}${line}.dictionary-trimmed.xml
